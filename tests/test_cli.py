...
def test_project_freeze():
    for _ in range(3):
        ...
        x=subprocess.run(f'minus80 freeze Project.foobar:{random_tag}'.split(),capture_output=True)
        assert x.returncode == 0

def test_project_freeze_tag_exists():
    x=subprocess.run(f'minus80 freeze Project.foobar:v1'.split(),capture_output=True)
    x=subprocess.run(f'minus80 freeze Project.foobar:v1'.split(),capture_output=True)
    assert x.returncode == 1

def test_project_freeze_no_tag():
    x=subprocess.run(f'minus80 freeze Project.foobar'.split(),capture_output=True)
    assert x.returncode == 1

def test_project_freeze_unavailable():
    x=subprocess.run(f'minus80 freeze Project.bizbaz:v1'.split(),capture_output=True)
    assert x.returncode == 1

def test_project_freeze_not_valid_freezable_type():
    x=subprocess.run(f'minus80 freeze foobar.bizbaz:v1'.split(),capture_output=True)
    assert x.returncode == 1
...
def test_project_thaw_no_tag():
    x = subprocess.run(f'minus80 thaw Project.foobar'.split())
    assert x.returncode == 1

def test_project_thaw_missing():
    x = subprocess.run(f'minus80 thaw Project.bizbaz'.split())
    assert x.returncode == 1

def test_project_thaw_missing_tag():
    x = subprocess.run(f'minus80 thaw Project.foobar:not_a_tag'.split())
    assert x.returncode == 1

def test_project_list():
    x=subprocess.run('minus80 list'.split())
    assert x.returncode == 0
 
def test_project_list_with_tags():
    x=subprocess.run('minus80 list --tags'.split())
    assert x.returncode == 0

def test_delete_project():
    tmpdir = tempfile.mkdtemp()
    x=subprocess.run(f'minus80 init --path {tmpdir} bizbaz'.split())
    assert m80.tools.available('Project','bizbaz')
    # Now delete the project
    x=subprocess.run(f'minus80 delete  Project.bizbaz --force'.split())
    assert not m80.tools.available('Project','bizbaz')
...
def test_delete_missing_dataset():
    ...
    x=subprocess.run(f'minus80 delete  Project.bazbaz --force'.split())
    assert x.returncode == 1
...
def test_delete_project_bad_tag():
    ...
    x=subprocess.run(f'minus80 delete  Project.bizbaz:v1 --force'.split())
