...
def require_private_key(fn, keyfile="minus80-e3336a5699a7.json", outdir="~"):
    """
    ...
    def wrapped(request):
        try:
            ...
            name = kms_client.crypto_key_path(
                "minus80", "global", "minus80_cloud_functions", "service_account"
            )
...
def list_datasets(request):
    ...
    db = firestore.client()
    # fetch the doc
    doc = db.document(f"Frozen/{uid}").get()
    # Create the response
    datasets = {}
    if doc.exists:
        datasets = doc.to_dict().get("available_datasets", {})
    return Response(
        response=json.dumps(datasets), status=200, mimetype="application/json"
    )
...
def stage_pull(request):
    ...
    db = firestore.client()
    dataset_ref = db.document(f"Frozen/{uid}/DatasetType/{dtype}/Dataset/{name}")
        ...
        if not tag_ref.exists:
            ...
            for tag_file in tag_data["files"].values():
                if tag_file["checksum"] not in files_on_client:
                    blob = bucket.blob(f'{uid}/{dtype}/{name}/{tag_file["checksum"]}')
...
def stage_files(request):
    ...
    if tag in available_tags:
        ...
        tag_data = (
            db.document(f"Frozen/{uid}/DatasetType/{dtype}/Dataset/{name}/Tag/{tag}")
            .get()
            .to_dict()
        )
        # set up the response variables
        status = 409
        response = {"tag_checksum": tag_data["total"]}
    else:
        ...
        response = {"stage_uuid": stage_uuid, "missing_files": [], "uploaded_files": []}
        status = 200
        ...
        for file_data in data["tag_data"]["files"].values():
            if (
                file_data["checksum"] not in dataset_files
                and file_data["checksum"] not in staged_files
            ):
...
def commit_file(request):
    ...
    staged_blob = staged_bucket.blob(f"{uid}/{dtype}/{name}/{stage_uuid}/{checksum}")
    ...
    try:
        if (
            ...
            or hashlib.sha256(staged_blob.download_as_string()).hexdigest() == checksum
        ):
            # Try again ...
            # Set up the target blob and transfer
            target_bucket = client.bucket("minus80")
            target_blob = target_bucket.blob(f"{uid}/{dtype}/{name}/{checksum}")
            ...
            dataset_ref.update({"available_files": firestore.ArrayUnion([checksum])})
            status = 200
            data = {}
        else:
            status = 409
...
def commit_tag(request):
    ...
    dataset_ref.update({"available_tags": firestore.ArrayUnion([tag])})
    tag_ref = db.document(f"Frozen/{uid}/DatasetType/{dtype}/Dataset/{name}/Tag/{tag}")
    ...
    # Return a response
    return Response(response=json.dumps({}), mimetype="application/json", status=200)
...
def create_dataset(dtype, name, owner_uid):
    ...
    """
    db = firestore.client()
    db.document(f"Frozen/{owner_uid}").set({"available_datasets": {}})
    dataset_ref = db.document(f"Frozen/{owner_uid}/DatasetType/{dtype}/Dataset/{name}")
    dataset = dataset_ref.get()
    if dataset.exists:
...
def get_uid(request):
    ...
    try:
        ...
        raise ValueError("Unable to extract uid") from e
    return uid
...
if __name__ == "__main__":
    ...
    if "GOOGLE_APPLICATION_CREDENTIALS" not in os.environ:
        raise ValueError(
            "Please set the env variable for GOOGLE_APPLICATION_CREDENTIALS"
        )
    ...
    app.run("127.0.0.1", 50000, debug=True, ssl_context=("cert.pem", "key.pem"))
